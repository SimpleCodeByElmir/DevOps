...
if ! [[ "$1" =~ ^[a-z]{1,7}$ ]]; then
		...
		if [ ${#DIRS_NAME} -eq 1 ]
		then
				echo "Notice: first parameter - only one letter inputed. So, ~215 dirs will be created with this one letter. Add more different letters to make it possible to generate much more files and faster."
		fi
fi
